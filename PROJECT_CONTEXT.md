...
Every route is a Server Component (no `page.tsx`/`layout.tsx` has `"use client"`). Interactivity lives in leaf/mid-level Client Components. Full detail: [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md).
...
Full per-component detail (props, state, consumers, risk level): [docs/FILE_MAP.md](docs/FILE_MAP.md). Client/Server boundary audit: [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md#client-server-boundary).
...
All content is typed by `src/content/types.ts`. Every page/component imports directly from the specific content file it needs (e.g. `@/content/projects`) — **there is no barrel re-export file** for content. Full data-flow map: [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md#data-architecture).
...
Full step-by-step lifecycle trace: [docs/ANIMATION_SYSTEM.md](docs/ANIMATION_SYSTEM.md#trailer-lifecycle).
...
Full detail (type scale, hover language, button/badge/card styles): [docs/DESIGN_SYSTEM.md](docs/DESIGN_SYSTEM.md).
...
Full "if X then edit Y" table with every field: [docs/CHANGE_GUIDE.md](docs/CHANGE_GUIDE.md).

---

## 18. High-Impact Files

Changing these ripples across many pages — always check consumers first (see [docs/FILE_MAP.md](docs/FILE_MAP.md) and [docs/CHANGE_GUIDE.md](docs/CHANGE_GUIDE.md#change-impact-map)):
...
3. **Search for all consumers** (`grep`/`Grep` for the export name or import path) — see [docs/FILE_MAP.md](docs/FILE_MAP.md) for a starting list per file.
...
Full recipes (add/remove a project, change tech stack, add a page, etc.): [docs/CHANGE_GUIDE.md](docs/CHANGE_GUIDE.md#common-change-recipes).
...
- **Verified-harmless static-export quirk:** the browser console shows 404s for a per-route React Server Components prefetch payload (this Next.js version's static export writes it under a different path than its own client router requests). Next.js's router catches this and falls back automatically — every route was confirmed to render correctly regardless. See [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md#static-export--github-pages).

Full detail: [docs/KNOWN_LIMITATIONS.md](docs/KNOWN_LIMITATIONS.md).
...
Full detail and suggested fixes: [docs/KNOWN_LIMITATIONS.md](docs/KNOWN_LIMITATIONS.md#technical-debt).

---

## Deployment

**Target: GitHub Pages, via GitHub Actions.** Workflow: [`.github/workflows/deploy.yml`](.github/workflows/deploy.yml). Full technical detail (exact `next.config.ts` settings, why each is needed, and everything verified): [docs/ARCHITECTURE.md § Static export / GitHub Pages](docs/ARCHITECTURE.md#11-static-export--github-pages).
...
| Document                                               | Covers                                                                                                                           |
| ------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------- |
| [docs/PROJECT_CONTEXT.md](docs/PROJECT_CONTEXT.md)     | Mirror of this file, for discoverability from `/docs`                                                                            |
| [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)           | Full route map, component architecture, data architecture, client/server boundary audit, config, dependencies, env vars, testing |
| [docs/FILE_MAP.md](docs/FILE_MAP.md)                   | Every source file: role, used-by, depends-on, modification risk                                                                  |
| [docs/CHANGE_GUIDE.md](docs/CHANGE_GUIDE.md)           | "If you want to change X, edit Y" table, change-impact map, safe-editing rules, common change recipes                            |
| [docs/DESIGN_SYSTEM.md](docs/DESIGN_SYSTEM.md)         | Colors, typography, spacing, radius, breakpoints, motion tokens, component visual language                                       |
| [docs/ANIMATION_SYSTEM.md](docs/ANIMATION_SYSTEM.md)   | Every motion primitive, the full Trailer lifecycle, reduced-motion handling                                                      |
| [docs/KNOWN_LIMITATIONS.md](docs/KNOWN_LIMITATIONS.md) | Real, code-verified limitations and technical debt                                                                               |
